...
function mod:oRA_MainTankUpdate()
	...
	if showmt then
		local newMTs = table.concat(mts, ",")
		local oldMTs = self.header:GetAttribute("nameList")
		if newMTs and oldMTs and newMTs == oldMTs then
			return
		end

		self.mainframe:Show()
		self.header:SetAttribute("nameList", newMTs)
		self.header:Show()
	else
		self.mainframe:Hide()
		self.header:Hide()
