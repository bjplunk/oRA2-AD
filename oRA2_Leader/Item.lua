...
local revision = tonumber(("$Revision: 664 $"):match("%d+"))
...
L:RegisterTranslations("enUS", function() return {
   ...
   DevoutCandle = "Sacred Candle",
   ArcanePowder = "Arcane Powder",
   WildSpineleaf = "Flintweed Seed",
   Ankh = "Ankh",
...
local items = {} -- need to initialise to something in case another addon sends an item request before the user initiates one
...
function mod:AddPlayer(nick, item, amount)
	...
	for i, v in ipairs(items) do
		if v[1] == nom then
			table.remove(items, i) -- another addon must be requesting items and we don't want the table to grow too large so remove dups
		end
	end
