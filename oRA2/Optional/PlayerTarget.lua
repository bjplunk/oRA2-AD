...
L:RegisterTranslations("koKR", function() return {
	...
	["Target"] = "대상",
	["TargetTarget"] = "대상의대상",
	["Nr of Players shown."] = "플레이어의 번호를 표시합니다.",
	["Nr Players"] = "플레이어의 수",
	["Classcolor"] = "직업별색상",
...
L:RegisterTranslations("frFR", function() return {
	...
	["Nr of Players shown."] = "Nombre de joueurs à afficher.",
	...
	["Order PT|PTT|PTTT Backwards."] = "Affiche la combinaison PT|PTT|PTTT \195\160 l'envers.",
...
L:RegisterTranslations("deDE", function() return {
	...
	["Target"] = "Ziel",
	["TargetTarget"] = "Ziel des Zieles",
	["Nr of Players shown."] = "Anzahl der angezeigten Spieler-Ziele.",
	["Nr Players"] = "Anzahl PlayerTargets",
	...
	["Set Player"] = "Setzt Spieler",
	["Set a player."]= "Setzt einen Spieler",
	["<nr> <name>"] = "<Nr> <Name>",
	["<nr>"] = "<Nr>",
	["<name>"] = "<Name>",
	["Remove Player"] = "Entferne Spieler",
...
function mod:oRA_PlayerTargetUpdate()
...
function mod:UpdateHealthBar(bar, unit)
	...
	if self.db.profile.classcolor then
		if not UnitIsEnemy(unit, "player") then 
			bar:SetStatusBarColor(select(2, paintchips:GetRGBPercent( select(2, UnitClass(unit)) )))
		else
...
function mod:UpdateConsole()
	for k = 1, 10, 1 do
		oRA.consoleOptions.args.pt.args.remove.args[k].name = tostring(k).."."
		oRA.consoleOptions.args.pt.args.set.args[k].name = tostring(k).."."
	end
	for k,v in pairs(self.db.profile.playertable) do
		if oRA:IsValidRequest(v,true) then
			oRA.consoleOptions.args.pt.args.remove.args[k].name = tostring(k)..". "..v
			oRA.consoleOptions.args.pt.args.set.args[k].name = tostring(k)..". "..v
		end
	end
end
